...
static DEFINE_PER_CPU(unsigned int, last_slot) = UINT_MAX;
static DEFINE_PER_CPU(int, last_tid) = -1;
...
static int input_event_pre_handler(struct kprobe *kp, struct pt_regs *regs)
{
    ...
    if (dev != touch_dev || type != EV_ABS)
        return 0;
    if (code == ABS_MT_SLOT) {
        this_cpu_write(last_slot, v);
        pr_info("handler SLOT: last_slot=%u state=%d\n", this_cpu_read(last_slot), atomic_read(&slot_state[this_cpu_read(last_slot)]));
    } else if (code == ABS_MT_TRACKING_ID) {
        unsigned int slot = this_cpu_read(last_slot);
        pr_info("handler TRACKING_ID start: v=%d last_slot=%u\n", v, slot);
        if (v >= 0 && v < MAX_TIDS) {
            this_cpu_write(last_tid, v);
            int old = atomic_cmpxchg(&slot_state[slot], SLOT_FREE, SLOT_HW);
            if (old == SLOT_SYN) {
