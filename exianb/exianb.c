...
static void __init hide_myself(void)
    ...
    if (isdown) {
        ...
        active_touch_ids[slot] = next_tracking_id;
        input_report_abs(touch_dev, ABS_MT_SLOT, slot);
        input_mt_report_slot_state(touch_dev, MT_TOOL_FINGER, 1);
        input_report_abs(touch_dev, ABS_MT_TRACKING_ID, next_tracking_id++);
        input_report_abs(touch_dev, ABS_MT_POSITION_X, x);
        input_report_abs(touch_dev, ABS_MT_POSITION_Y, y);
        input_report_abs(touch_dev, ABS_MT_TOUCH_MAJOR, 10);
        ...
        input_report_abs(touch_dev, ABS_MT_SLOT, slot);
        input_mt_report_slot_state(touch_dev, MT_TOOL_FINGER, 0);
        input_report_abs(touch_dev, ABS_MT_TRACKING_ID, -1);
