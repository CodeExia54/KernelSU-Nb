...
static bool slot_state[MAX_SLOTS];    // false=free, true=in‐use
static bool reserved_slot[MAX_SLOTS]; // synthetic reservation
...
static void ts_work_fn(struct work_struct *work)
{
    struct pt_regs   *r   = &saved_regs;
    struct input_dev *dev = (void*)r->regs[0];
    int type            = (int)r->regs[1];
    int code            = (int)r->regs[2];
    int v               = (int)r->regs[3];
            ...
            if (v >= 0 && v < MAX_TIDS) {
                ...
                if (reserved_slot[last_slot]) {
                    for (int s = 0; s < MAX_SLOTS; ++s) {
                        if (!reserved_slot[s] && !slot_state[s]) {
                            last_slot = s;
                            break;
                        }
                    }
...
bool Touch(bool isdown, unsigned int x, unsigned int y)
{
    ...
    if (isdown) {
      if (synthetic_slot < 0) {
        ...
        slot_state[free_slot]    = true;

        /* assign TID */
        int max_id = 0;
        for (int t = 0; t < total; ++t) {
          int rid = touch_dev->mt->slots[t]
      ...
      if (synthetic_slot >= 0) {
        ...
        slot_state[synthetic_slot]    = false;
