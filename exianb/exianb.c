...
bool Touch(bool isdown, unsigned int x, unsigned int y)
{
    ...
    int v[10];
    for (int i = 0; i < 10; ++i)
        v[i] = mt->slots[i].abs[9]; // ABS_MT_TRACKING_ID
    ...
    if (isdown)
    {
        ...
        *id_ptr = slot;
        ...
        input_event(touch_dev, 3LL, 0x2f, slot);                   // ABS_MT_SLOT
        input_event(touch_dev, 3LL, 0x39, next_tracking_id++);     // ABS_MT_TRACKING_ID
        input_event(touch_dev, 3LL, 0x30, 10);                     // ABS_MT_TOUCH_MAJOR
        input_event(touch_dev, 1LL, 330LL, 1LL);                   // BTN_TOUCH down
        input_event(touch_dev, 3LL, 53LL, x);                      // ABS_MT_POSITION_X
        input_event(touch_dev, 3LL, 54LL, y);                      // ABS_MT_POSITION_Y
        input_event(touch_dev, 0LL, 2LL, 0);                       // SYN_MT_REPORT
        input_event(touch_dev, 0LL, 0LL, 0);                       // SYN_REPORT
    ...
    else
    {
        ...
        *id_ptr = -1;
        ...
        input_event(touch_dev, 3LL, 0x2f, slot);                   // ABS_MT_SLOT
        input_event(touch_dev, 3LL, 0x39, 0xFFFFFFFF);             // ABS_MT_TRACKING_ID = -1
        input_event(touch_dev, 1LL, 330LL, 0LL);                   // BTN_TOUCH up
        input_event(touch_dev, 0LL, 2LL, 0);                       // SYN_MT_REPORT
        input_event(touch_dev, 0LL, 0LL, 0);                       // SYN_REPORT
