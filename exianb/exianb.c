...
static int pop_free_slot(void){
    int slot = -1;
    spin_lock(&slot_lock);
    if(free_head > 0) slot = free_slots[--free_head];
...
static int input_event_pre_handler(struct kprobe *kp, struct pt_regs *regs){
    ...
    int type = (int)regs->regs[1], code = (int)regs->regs[2], v = (int)regs->regs[3];
    if(dev != touch_dev || type != EV_ABS || !injecting) return 0;
    if(code == ABS_MT_SLOT && v == synthetic_slot){
        int new_slot = pop_free_slot();
        if(new_slot >= 0) regs->regs[3] = new_slot;
    }
    return 0;
}
/*
...
bool Touch(bool isdown, unsigned int x, unsigned int y){
    ...
    if(isdown){
        slot = pop_free_slot();
        if(slot < 0){ injecting = false; mutex_unlock(&touch_mutex); return false; }
        synthetic_slot = slot;
        tid = next_tracking_id++;
        active_touch_ids[slot] = tid;
