...
/* pop one slot—atomically under spinlock */
...
static int input_event_pre_handler(struct kprobe *kp, struct pt_regs *regs)
{
    ...
    /* only our touch_dev, only ABS events */
    if (dev != touch_dev || type != EV_ABS)
        return 0;

    /* skip *only* the synthetic-slot / synthetic-TID events */
    if (injecting && (code == ABS_MT_SLOT || code == ABS_MT_TRACKING_ID))
        return 0;

    /* whenever hardware says “new finger down”… */
    if (code == ABS_MT_TRACKING_ID && v >= 0) {
        int slot = pop_free_slot();
        if (slot < 0)
            return 0;               // no free slots left
        /* rewrite slot so the core/Apps see `slot` instead of the driver’s original */
        regs->regs[2] = ABS_MT_SLOT;
        regs->regs[3] = slot;
    }
    /* whenever hardware says “finger up”… */
    else if (code == ABS_MT_TRACKING_ID && v == -1) {
        /* the driver always prefixes UP with a SLOT event */
        int last_slot = regs->regs[3];
        push_free_slot(last_slot);
    }
...
bool Touch(bool isdown, unsigned int x, unsigned int y)
{
    ...
    if (isdown) {
        ...
        /* synthetic will own this slot & TID */
        synthetic_slot = slot;
        tid            = next_tracking_id++;

        /* mark “we’re injecting” so pre-handler skips only these two calls */
        injecting = true;
        ...
        /* same trick: only skip our two slot/TID calls */
        injecting = true;
        input_event(touch_dev, EV_ABS, ABS_MT_SLOT,        slot);
        ...
        /* free the slot back to the pool */
        push_free_slot(slot);
        synthetic_slot = -1;
    }
