...
bool Touch(bool isdown, unsigned int x, unsigned int y){
    ...
    if (isdown) {
        if (synthetic_slot < 0) {
            for (synthetic_slot = 0; synthetic_slot < 10; ++synthetic_slot) {
                if (mt->slots[synthetic_slot].abs[9] < 0)
                    break;
            }
            if (synthetic_slot >= 10) {
                mutex_unlock(&touch_mutex);
                return false;
            }
            // --- avoid any existing ID collisions ---
    int max_id = 0;
    for (int i = 0; i < 10; ++i) {
        int id = mt->slots[i].abs[9];
        if (id > max_id)
            max_id = id;
    }
    ...
    int tracking_id = next_tracking_id++;
            ...
            input_event(touch_dev, 3LL, 47LL, synthetic_slot); // ABS_MT_SLOT
            input_event(touch_dev, 3LL, 57LL, tracking_id);    // ABS_MT_TRACKING_ID
        } else {
            input_event(touch_dev, 3LL, 47LL, synthetic_slot); // use existing
        }
        ...
        input_event(touch_dev, 3LL, 58LL, 30LL); // ABS_MT_PRESSURE
        input_event(touch_dev, 3LL, 48LL, 30LL); // ABS_MT_WIDTH_MAJOR
        input_event(touch_dev, 1LL, 330LL, 1LL); // BTN_TOUCH down
        input_event(touch_dev, 0LL, 0LL, 0LL);   // SYN_REPORT
        mutex_unlock(p_mutex);
        mutex_unlock(&touch_mutex);
        return true;
    } else {
        ...
        mutex_lock(p_mutex);
        input_event(touch_dev, 3LL, 47LL, synthetic_slot); // ABS_MT_SLOT
        input_event(touch_dev, 3LL, 57LL, -1LL);           // RELEASE
        input_event(touch_dev, 1LL, 330LL, 0LL);           // BTN_TOUCH up
        input_event(touch_dev, 0LL, 0LL, 0LL);             // SYN_REPORT
