...
bool Touch(bool isdown, unsigned int x, unsigned int y)
{
    ...
    if (isdown) {
        ...
        if (current_slot < 0) {
            ...
            active_touch_ids[current_slot] = current_slot;
        }

        /* send down/move */
        mutex_lock(&touch_dev->mutex);
        input_event(touch_dev, EV_ABS, ABS_MT_SLOT,        current_slot);
        input_event(touch_dev, EV_ABS, ABS_MT_TRACKING_ID, current_slot);
...
bool Touch(bool isdown, unsigned int x, unsigned int y)
{
    ...
    if (isdown)
    {
        ...
        *id_ptr = slot;
        ...
        // ----- START PATCH -----
        /* 1) tell the driver which MT slot, and give it a tracking‑ID */
        input_event(touch_dev, EV_ABS, ABS_MT_SLOT,            slot);
        input_event(touch_dev, EV_ABS, ABS_MT_TRACKING_ID,     slot);
        /* 2) now do your normal major/position/pressure */
        input_event(touch_dev, 3LL,     47LL,                  10LL); // ABS_MT_TOUCH_MAJOR
        input_mt_report_slot_state(touch_dev, slot,           1LL); // BTN_TOUCH down
        input_event(touch_dev, 3LL,     53LL,                  x);   // ABS_MT_POSITION_X
        input_event(touch_dev, 3LL,     54LL,                  y);   // ABS_MT_POSITION_Y
        input_event(touch_dev, 3LL,     58LL,                  30LL);// ABS_MT_PRESSURE
        input_event(touch_dev, 3LL,     48LL,                  30LL);// ABS_MT_WIDTH_MAJOR
        /* 3) sync the MT frame, then the overall frame */
        input_mt_sync(touch_dev);
        input_sync(touch_dev);
        // ----- END PATCH -----
    ...
    else
    {
        ...
        *id_ptr = slot;
        ...
        // ----- START PATCH -----
        /* 1) select the same slot, end its tracking */
        input_event(touch_dev, EV_ABS, ABS_MT_SLOT,        slot);
        input_event(touch_dev, EV_ABS, ABS_MT_TRACKING_ID, -1);
        input_mt_report_slot_state(touch_dev, slot,       0LL); // BTN_TOUCH up
        /* 2) sync it out */
        input_mt_sync(touch_dev);
        input_sync(touch_dev);
        // ----- END PATCH -----
