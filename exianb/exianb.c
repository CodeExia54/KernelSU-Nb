...
/* --- buffer_until_syn input-handler code start --- */
struct buf_event {
    struct buf_event *next;
    unsigned int type;
    unsigned int code;
    int value;
};

static struct buf_event *buf_head;

static int buf_event(struct input_handle *handle,
                     unsigned int type,
                     unsigned int code,
                     int value)
{
    struct buf_event *old, *e, *rev, *tmp;

    /* only buffer your touch_dev */
    if (handle->dev != touch_dev)
        return 0;

    /* flush on each SYN_REPORT */
    if (type == EV_SYN && code == SYN_REPORT) {
        pr_info("buffer: SYN_REPORT seen, flushing events, head=%p\n", buf_head);
        old = xchg(&buf_head, NULL);
        rev = NULL;
        while (old) {
            tmp = old->next;
            old->next = rev;
        ...
        while (rev) {
            tmp = rev->next;
            input_event(handle->dev, rev->type, rev->code, rev->value);
            kfree(rev);
            rev = tmp;
        }
        input_sync(handle->dev);
        pr_info("buffer: flushed events\n");
        return 0;
    }
    ...
    e->code = code;
    e->value = value;
    pr_info("buffer: queued event type=%u code=%u value=%d\n", type, code, value);
    do {
        old = READ_ONCE(buf_head);
        e->next = old;
    } while (cmpxchg(&buf_head, old, e) != old);
    return 1;
}

static int buf_connect(struct input_handler *handler,
                       ...
                       const struct input_device_id *id)
{
    ...
    snprintf(handle->name, sizeof(handle->name), "buf_%s", dev->name);
    pr_info("buffer: connected to device %s\n", dev->name);
...
static void buf_disconnect(struct input_handle *handle)
{
    pr_info("buffer: disconnected from device %s\n", handle->dev->name);
    input_close_device(handle);
    input_unregister_handle(handle);
    kfree(handle);
}

static const struct input_device_id buf_ids[] = {
    { .driver_info = 1 },
    { }
};

MODULE_DEVICE_TABLE(input, buf_ids);

static struct input_handler buf_handler = {
    .event      = buf_event,
    .connect    = buf_connect,
    .disconnect = buf_disconnect,
    .name       = "buffer_until_syn",
    .id_table   = buf_ids,
};
/* --- buffer_until_syn input-handler code end --- */
...
#if 0 /* disable input_event_pre_handler for testing */
