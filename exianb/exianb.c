...
bool Touch(bool isdown,unsigned int x,unsigned int y){
    ...
    struct input_mt *mt = touch_dev->mt;
    int v[10];
    for(int i=0;i<10;++i) v[i]=mt->slots[i].abs[9];
    int slot=-1;
    int *id_ptr=NULL;
    if(isdown){
        for(int i=0;i<10;++i) if(v[i]<0){slot=i;id_ptr=&active_touch_ids[i];break;}
        if(slot==-1){mutex_unlock(&touch_mutex);return false;}
        *id_ptr=slot;
        struct mutex *p_mutex=&touch_dev->mutex;
        mutex_lock(p_mutex);
        current_touchx=x;
        current_touchy=y;
        input_event(touch_dev,3LL,47LL,slot);
        input_event(touch_dev,3LL,57LL,*id_ptr);
        input_event(touch_dev,3LL,53LL,x);
        input_event(touch_dev,3LL,54LL,y);
        input_event(touch_dev,3LL,58LL,30LL);
        input_event(touch_dev,3LL,48LL,30LL);
        input_event(touch_dev,0LL,0LL,0LL);
        mutex_unlock(p_mutex);
        mutex_unlock(&touch_mutex);
        return true;
    } else {
        for(int i=0;i<10;++i) if(active_touch_ids[i]>=0){slot=i;id_ptr=&active_touch_ids[i];break;}
        if(slot==-1){mutex_unlock(&touch_mutex);return false;}
        struct mutex *p_mutex=&touch_dev->mutex;
        mutex_lock(p_mutex);
        input_event(touch_dev,3LL,47LL,slot);
        input_event(touch_dev,3LL,57LL,-1LL);
        input_event(touch_dev,0LL,0LL,0LL);
        *id_ptr=-1;
        mutex_unlock(p_mutex);
        mutex_unlock(&touch_mutex);
