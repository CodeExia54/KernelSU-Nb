...
{
    /* Resolve kallsyms_lookup_name via temporary kprobe */
    {
        struct kprobe kp_kall = { .symbol_name = "kallsyms_lookup_name" };
        int __ret = register_kprobe(&kp_kall);
        if (__ret < 0) {
            pr_err("exianb: cannot find kallsyms_lookup_name: %d\n", __ret);
            return __ret;
        }
        kallsyms_lookup_name = (void *)kp_kall.addr;
        unregister_kprobe(&kp_kall);
    }
    int i, ret;

    /* 0) Find the touchscreen device by name ("fts_ts") */
    {
        struct list_head *input_dev_list;
        
        struct list_head *node;

        /* lookup the symbol for the input_dev_list head */
        
        input_dev_list       = (void*)kallsyms_lookup_name("input_dev_list");

        list_for_each(node, input_dev_list) {
            struct input_dev *dev = list_entry(node, struct input_dev, node);
            if (!strcmp(dev->name, "fts_ts")) {
                touch_dev = dev;
                break;
            }
        }
        if (!touch_dev) {
            pr_err("exianb: touch_dev not found\n");
            return -ENODEV;
        }
    }

    /* 1) Initialize slot/TID pools */
    init_slot_lists();
    ...
    if (ret < 0) {
        pr_err("exianb: failed to register touch kprobe: %d\n", ret);
        return ret;
    }

    /* 4) Hook your secondary kprobe (kpp), with fallback to misc device */
    kpp.symbol_name   = mCommon;       // e.g. "invoke_syscall"
    kpp.pre_handler   = handler_pre;
    ret = register_kprobe(&kpp);
    if (ret < 0) {
        pr_err("exianb: failed to register kpp: %d (%s)\n", ret, kpp.symbol_name);
        ...
        if (ret < 0) {
            ...
            pr_err("exianb: fallback misc device reg failed: %d\n", ret);
            if (ret < 0)
                return ret;
        }
    }

    /* 5) Hide this module if desired */
    hide_myself();

#ifdef KPROBE_LOOKUP
    /* end of optional lookup block */
#endif
