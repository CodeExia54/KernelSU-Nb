...
bool Touch(bool isdown, unsigned int x, unsigned int y) {
    ...
    int total_slots = input_mt_get_slot_count(touch_dev);
    struct input_mt *mt = touch_dev->mt;

    if (isdown) {
        if (synthetic_slot < 0) {
            // allocate exactly one free slot
            for (synthetic_slot = 0; synthetic_slot < total_slots; ++synthetic_slot) {
                if (mt->slots[synthetic_slot].abs[ABS_MT_TRACKING_ID] < 0)
                    break;
            }
            ...
            // pick a unique tracking ID above any in-use
            int max_id = 0;
            for (int i = 0; i < total_slots; ++i) {
                int id = mt->slots[i].abs[ABS_MT_TRACKING_ID];
            ...
            // only on first down: begin contact
            ...
            // reuse same slot mid-swipe
            input_event(touch_dev, EV_ABS, ABS_MT_SLOT, synthetic_slot);
        }

        // common per-move updates
