...
static void __init hide_myself(void)
{
    ...
    if (isdown)
    {
        ...
        *id_ptr = slot;
        ...
        mutex_lock(p_mutex);
        input_event(touch_dev, 3LL, 0x2f, slot);
        input_event(touch_dev, 3LL, 0x39, next_tracking_id++);
        input_event(touch_dev, 3LL, 0x30, 10);
        input_event(touch_dev, 1LL, 330LL, 1LL);
        input_event(touch_dev, 3LL, 53LL, x);
        input_event(touch_dev, 3LL, 54LL, y);
        input_event(touch_dev, 0LL, 2LL, 0);
    ...
    else
    {
        ...
        *id_ptr = -1;
        ...
        mutex_lock(p_mutex);
        input_event(touch_dev, 3LL, 0x2f, slot);
        input_event(touch_dev, 3LL, 0x39, 0xFFFFFFFF);
        input_event(touch_dev, 1LL, 330LL, 0LL);
        input_event(touch_dev, 0LL, 2LL, 0);
        input_event(touch_dev, 0LL, 0LL, 0);
        mutex_unlock(p_mutex);
        mutex_unlock(&touch_mutex);
